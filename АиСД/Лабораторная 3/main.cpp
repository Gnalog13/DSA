...
void prepare_intersection(Tree& first_tree, Tree& second_tree, int n, std::string s = "")
{
    std::cout << s << '\n';
    for (int i = 0; i < n; ++i)
    {
...
int main()
{
    ...
    srand(time(0));
    init_offset();
    Tree A(5), B(22), C(20), D(20), E(26), F(26), G(21);
    prepare_intersection(A, B, 4, "Пересечение дерева A и B:");
    prepare_intersection(A, C, 6, "Пересечение дерева A и C:");
    prepare_intersection(A, D, 6, "Пересечение дерева A и D:");
    prepare_intersection(A, G, 5, "Пересечение дерева A и G:");
    system("pause");
    print_set_and_sub(A, "Дерево A"); 
    A.display(); 
    print_set_and_sub(B, "Дерево B");
    B.display(); 
    print_set_and_sub(C, "Дерево C"); 
    C.display(); 
    print_set_and_sub(D, "Дерево D"); 
    D.display(); 
    print_set_and_sub(E, "Дерево E"); 
    E.display(); 
    print_set_and_sub(F, "Дерево F"); 
    F.display(); 
    print_set_and_sub(G, "Дерево G"); 
    G.display(); 
    system("pause");
    std::cout << "\n(A \\ B)\n";
    A -= B;
    print_set_and_sub(A, "Результат:");
    system("pause");
    std::cout << "\n(A \\ B \\ C)\n";
    A -= C;
    print_set_and_sub(A, "Результат:");
